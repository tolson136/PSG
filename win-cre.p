...
def var zcust like acct-rcv.cust label "Generate completed orders for customer ".
...
THEN DO:
    ...
    RETURN.
END.
    UPDATE BEG# D5 SKIP(5) zcust skip(5) WITH NO-BOX SIDE-LABELS.
   FOR EACH ACCT-RCV /* USE-INDEX INDX1 */
                     WHERE ACCT-RCV.COMP# = XCOM AND
                           ACCT-RCV.DIV# = XDIV AND
                           ACCT-RCV.C-STATUS <> "I"
                           BY ACCT-RCV.CUST#:
   if zcust > 0 and acct-rcv.cust# <> zcust then next.
...
END.
   ...
   IF NOT AVAILABLE PRO-DESP THEN DO:
       HIDE MESSAGE.
       MESSAGE "PROPSL # " PRO-DESP.ITEM# " NOT AVAILABLE FOR PROCESSING".
       UNDO, NEXT.
   END.
