...
REPEAT:
DISPLAY SPACE(13)
"I N V O I C E   D E S C R I P T I O N   S C R E E N         (ADD)"
SKIP(1)
    ...
    FIND ACCT-RCV WHERE ACCT-RCV.CUST# = XCUST AND ACCT-RCV.COMP# = XCOM AND
                        ACCT-RCV.DIV# = XDIV AND ACCT-RCV.C-STATUS <> "I"
                        NO-ERROR.
    IF NOT AVAILABLE ACCT-RCV
    THEN DO:
       MESSAGE "CUSTOMER # " XCUST
    ...
    THEN DO:
       ...
       UNDO, RETRY.
    END.
    DISPLAY PROPSL.PROPSL# LABEL "PROPOSAL #" SKIP(1).
    PROMPT-FOR PRO-DESP.ITEM#.
    XITEM = INPUT PRO-DESP.ITEM#.
    FIND PRO-DESP WHERE PRO-DESP.COMP# = XCOM AND PRO-DESP.DIV# = XDIV AND
                      PRO-DESP.CUST# = XCUST AND PRO-DESP.PROPSL# = XPROP AND
    ...
    THEN DO:
       ...
       UNDO, RETRY.
    END.
    DISPLAY PRO-DESP.ITEM# LABEL "ITEM #" SKIP(1).
    ...
    IF AVAILABLE AR-DESP
    THEN DO:
        MESSAGE "THIS INVOICE DESCRIPTION ALREADY EXISTS, TRY CHANGING IT".
        UNDO, RETRY.
    END.
    CREATE AR-DESP.
    UPDATE AR-DESP.COMP# = XCOM.
    UPDATE AR-DESP.DIV# = XDIV.
    UPDATE AR-DESP.PROPSL# = XPROP.
    UPDATE AR-DESP.ITEM# = XITEM.
    UPDATE AR-DESP.CUST# = XCUST.
    DISPLAY AR-DESP.DESC01 LABEL "DESCRIPTION"
